...
## 🎬 5-Minute Hackathon Demo Click Sequence

When presenting or recording a pitch video of Payment Proof:
1. **Command Center**: Open `http://localhost:3000`. Show the operations triage stream across merchants (Swiggy, Zomato, BookMyShow) and the 8 active discrepancies.
2. **Open Hero Incident (`inc_test_001`)**: Select the Swiggy ₹4,500 Ghost Debit incident.
3. **Run Forensic Investigation**: Click *"Run Forensic Investigation"*. Point to the **Contradictions Banner** highlighting the direct conflict between Bank success and Gateway timeout.
4. **Multi-Party Truth Matrix**: Walk through the 4 columns: Bank switch (SUCCESS, UTR generated), Gateway (FAILED, 65s timeout), Merchant OMS (CANCELLED), Webhook (DROPPED, HTTP 504).
5. **Timeline Chronology**: Click the **Timeline** tab. Show the top trajectory bar and explain the exact millisecond sequence of how the transaction broke.
6. **ML Assessment & Java Safety Decision**: Click the **AI Report** tab. Show that the Random Forest model predicted `BANK_DEBIT_GATEWAY_FAILURE` (99.2% confidence), but highlight the red lock: Java strictly prohibited checkout retry because customer funds were debited.
7. **Gemini Forensic Narrative**: Show the plain-English executive summary generated for support agents.
8. **Resolution Center**: Navigate to **Resolution**. Click *"Issue Source Refund"* (₹4,500.00).
9. **Audit Log Verification**: Click **Audit Log**. Click *"Verify Cryptographic Integrity"* to demonstrate the tamper-proof SHA-256 blockchain-style mathematical proof.
10. **Closing**: *"When payment systems disagree, merchants don't just need a failure status. They need the truth."*
